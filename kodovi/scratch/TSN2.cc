...
uint32_t m_tsn_bytes_received = 0;
uint32_t m_be_bytes_received = 0;

uint32_t m_tsn_packets_received = 0;
uint32_t m_be_packets_received = 0;
...
ReceivedPacket(Ptr<const Packet> p)
{
    ...
    */
    ...
    if (trafficType == "TSN")
    {
        ...
        tsnCount++;
        m_tsn_bytes_received += p->GetSize();
        m_tsn_packets_received++;
    }
    else if (trafficType == "BE")
    {
        beDelay += packetDelay;
        beCount++;
        m_be_bytes_received += p->GetSize();
...
void Ratio()
{
    ...
    if (duration > 0)
    {
        double tsn_throughput_bps = (m_tsn_bytes_received * 8.0) / duration;
        double be_throughput_bps = (m_be_bytes_received * 8.0) / duration;

        std::cout << "\n--- Per Traffic Type Throughput ---" << std::endl;
        std::cout << "TSN Throughput: " << tsn_throughput_bps << " bps (" << tsn_throughput_bps / 1000.0 << " kbps)" << std::endl;
        std::cout << "BE Throughput : " << be_throughput_bps << " bps (" << be_throughput_bps / 1000.0 << " kbps)" << std::endl;

        std::cout << "TSN Packets Received: " << m_tsn_packets_received << std::endl;
        std::cout << "BE Packets Received : " << m_be_packets_received << std::endl;
    }
}
...
int main(int argc, char *argv[])
{
    ...
    tsnHelperClient.Uninstall(dev13.Get(0));
    tsnHelperServer.Uninstall(dev13.Get(1));
    QueueDiscContainer qdiscsClient1 = tsnHelperClient.Install(dev13.Get(0));
    QueueDiscContainer qdiscsServer = tsnHelperServer.Install(dev13.Get(1));
    ...
    tsnApp.Stop(Seconds(simulationTime));
    ...
    beApp.Stop(Seconds(simulationTime));
